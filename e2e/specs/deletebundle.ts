...
var testdata = require('../inputs/testdata/deletebundle.json');


describe('widget', function () {
  it('deleting bundle from its details page', function () {
...
browser.actions().mouseMove(element(by.xpath("//div[@class='control-bar visible-md-up hidden-sm hidden-xs']/ecb-contextbar/ecb-modal-dialog[1]/div/div/div/div[2]/div/button[1]/dialog-button-1"))).perform();
   ...
   console.log('popup is not came for delete');
       ...
       var error = element(by.xpath("//ecb-bundle/div[1]/div[1]/div/div/p"));
      error.isPresent().then(function (result){
          if(result){
               browser.actions().mouseMove(element(by.xpath("//ecb-bundle/div[1]/div[1]/div/div/p"))).perform();
          browser.sleep(3000);
          element(by.xpath("//ecb-bundle/div[1]/div[1]/div/div/span/i")).click();
          browser.sleep(4000);
          } else {
         ...
         console.log('delete button is not there in bundle detail page');
              }
});
