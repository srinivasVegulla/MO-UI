var testdata = require('../inputs/testdata/discountdetailpageinpitemplates.json');

import { browser, by, element } from 'protractor';

describe('widget', function () {
    it('pitemplates discount detail page ', function() {
                 browser.sleep(5000);
                var heading = element(by.xpath("//ecb-breadcrumb/div[3]/h1"));
                heading.isPresent().then (function (result) {
            if (result) {
                 expect(element(by.xpath("//ecb-breadcrumb/div[3]/h1")).getText()).toEqual(testdata.discountheading);
                 browser.sleep(2000);
